...
One command, six tools, plus an `all` pipeline. Works the same on Windows,
Linux and Android/Termux (pure stdlib).

  python wigle.py analyze CAPTURE.csv [--oui oui.json] [--json out.json]
  python wigle.py cluster CAPTURE.csv [--include-ble] [--min-bssids 3]
  python wigle.py track   CAPTURE.csv [--min-trips 3] [--min-sep-km 0.5]
  python wigle.py map     CAPTURE.csv [--out survey]
  python wigle.py report  CAPTURE.csv [--out report.html]
